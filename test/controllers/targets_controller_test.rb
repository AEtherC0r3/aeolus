...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { room_id: @target.room_id, type: @target.type, value: @target.value } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { room_id: @target.room_id, type: @target.type, value: @target.value } }
